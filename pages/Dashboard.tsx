      ...
      <div className="max-w-[1600px] mx-auto space-y-16">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-12 border-b border-slate-200 pb-16">
            ...
            <div className="flex items-center gap-8">
                <div className="flex items-center gap-6 bg-white px-8 py-4 rounded-[2.5rem] border border-slate-100 shadow-luxury group cursor-pointer">
                    <div className="h-14 w-14 bg-[#064e3b] rounded-[1.5rem] flex items-center justify-center text-white text-xl font-black">{userData?.name?.charAt(0)}</div>
                    <div className="hidden sm:block">
                      <span className="text-sm font-black text-slate-900 block leading-none uppercase tracking-tight">{userData?.name}</span>
                      <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-[0.3em] mt-2 block">Gestor Municipal</span>
                    </div>
                </div>
            </div>
        </header>
            ...
            <div className="col-span-12 xl:col-span-8 space-y-12">
                <div className="bg-white rounded-[3.5rem] border border-slate-100 p-16 shadow-luxury relative overflow-hidden group">
                    <div className="flex justify-between items-center mb-16 relative z-10">
                        <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter flex items-center gap-4"><Zap className="h-7 w-7 text-emerald-600" /> Módulos Estratégicos</h3>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-8 relative z-10">
                        {[
                            ...
                            { icon: Building, title: "Gestão de Rede", desc: "Base nominal profissional e física.", to: "/admin/escolas?tab=students", color: "text-blue-600 bg-blue-50" },
                            ...
                            <Link key={i} to={mod.to} className="flex items-center p-8 bg-slate-50/50 rounded-[2.5rem] hover:bg-white hover:shadow-deep border border-transparent hover:border-slate-100 transition-all group">
                                <div className={`p-5 rounded-[1.8rem] ${mod.color} mr-8 shadow-xl transition-all duration-500`}><mod.icon className="h-8 w-8" /></div>
                                ...
                                <ChevronRight className="h-6 w-6 text-slate-300 group-hover:text-emerald-600 transition-all" />
                            </Link>
                        ))}
                    </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-12">
                  <div className="card-requinte !p-16 flex flex-col justify-between group overflow-hidden relative">
                    <div className="relative z-10">
                      <div className="flex items-center gap-5 mb-10">
                        <div className="bg-blue-600 p-4 rounded-2xl text-white shadow-xl"><MapPin className="h-6 w-6" /></div>
                        <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Logística Geo</h3>
                      </div>
                      <p className="text-sm font-medium text-slate-500 uppercase tracking-widest mb-10">Raio médio de alocação:</p>
                      <div className="flex items-baseline gap-4">
                        <span className="text-7xl font-black text-slate-900 tracking-tighter">{stats.avgDistance.toFixed(1)}</span>
                        <span className="text-xl font-black text-blue-600 uppercase tracking-widest">KM</span>
                      </div>
                    </div>
                  ...
                  <div className="bg-emerald-600 rounded-[3.5rem] p-16 text-white shadow-deep relative overflow-hidden group">
                     <div className="relative z-10 flex flex-col h-full justify-between">
                        <ShieldCheck className="h-10 w-10 text-emerald-300 mb-10" />
                        <h3 className="text-3xl font-black tracking-tighter uppercase">Integridade <br/>Auditada MEC.</h3>
                        <p className="text-emerald-100/60 text-sm mt-6 font-medium">Sincronismo Nominal Ativo</p>
                     </div>
                  </div>
                </div>
            </div>
            ...
            <div className="col-span-12 xl:col-span-4 space-y-12">
                ...
                <div className="card-requinte !p-16 space-y-12">
                    <div className="flex justify-between items-center border-b border-slate-100 pb-8">
                        <h3 className="text-[13px] font-black text-slate-900 uppercase tracking-[0.5em] flex items-center gap-5"><Activity className="h-6 w-6 text-emerald-600" /> Saúde da Rede</h3>
