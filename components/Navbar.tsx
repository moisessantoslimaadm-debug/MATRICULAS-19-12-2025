...
import { GraduationCap, Menu, X, CloudOff, LogOut, ShieldCheck } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  const isActive = (path: string) => 
    location.pathname === path 
      ? 'text-emerald-700 font-bold border-b-2 border-emerald-600 px-3 py-4 text-[10px] uppercase tracking-wider' 
      : 'text-slate-500 hover:text-emerald-700 font-semibold px-3 py-4 transition-all text-[10px] uppercase tracking-wider';
          ...
          <div className="hidden lg:flex items-center">
            {(role === UserRole.ADMIN_SME || role === UserRole.DIRECTOR) && (
              <>
                ...
                <Link to="/admin/map" className={isActive('/admin/map')}>Geo</Link>
                <Link to="/admin/data" className={isActive('/admin/data')}>Censo</Link>
                <Link to="/reports" className={isActive('/reports')}>BI</Link>
              </>
            )}
            {!role && (
                <>
        ...
        <div className="flex items-center gap-4">
          ...
          {role ? (
            <div className="flex items-center gap-3 border-l pl-4 border-slate-200">
              ...
              <button onClick={handleLogout} className="p-1.5 text-slate-400 hover:text-red-600 transition-colors">
